...
const auto MAX_PEOPLE = RENDER_MAX_PEOPLE;  // defined in render_functions.hpp = 96/
const auto BOX_SIZE = 368;
const auto BUFFER_SIZE = 4;    //affects latency
const auto MAX_NUM_PARTS = 70;  // 14+1+13*2 = 41
...
void warmup(int device_id) {
    ...
    } else if (net_copies[device_id].nms_num_parts==15) {
...
void render(int gid, float *heatmaps /*GPU*/) { //todo to change
    float* centers = 0;
    float* poses    = net_copies[gid].joints;

    double tic = get_wall_time();

    if (net_copies[gid].up_model_descriptor->get_number_parts()==14) {
        render_AI_parts(net_copies[gid].canvas, DISPLAY_RESOLUTION_WIDTH, DISPLAY_RESOLUTION_HEIGHT, NET_RESOLUTION_WIDTH, NET_RESOLUTION_HEIGHT,
        heatmaps, BOX_SIZE, centers, poses, net_copies[gid].num_people, global.part_to_show);

    } else if (net_copies[gid].up_model_descriptor->get_number_parts()==15) {
        ...
        heatmaps, BOX_SIZE, centers, poses, net_copies[gid].num_people, global.part_to_show); //part_to_show = 
...
void* getFrameFromDir(void *i) {
    ...
    while(1) {
        ...
        process_and_pad_image(frame.data_for_mat, image_uchar, DISPLAY_RESOLUTION_WIDTH, DISPLAY_RESOLUTION_HEIGHT, 0);// todo 
        ...
        for(int i=0; i < BATCH_SIZE; i++) {
            ...
            target_width = 16 * ceil(NET_RESOLUTION_WIDTH * scale /16); //todo why 16
        ...
        global.input_queue.push(frame); //todo frame  data for batch size image_temp
...
int connectLimbs(
    ...
    ModelDescriptor *model_descriptor) {
        ...
        int SUBSET_SIZE = num_parts+3; //each subset for one person
        ...
        subset.clear();
        connection.clear();
// map_x for point x to heatmap
        for(int k = 0; k < number_limb_seq; k++) { // for each limb to bipartite
            ...
            const int num_inter = 10; //interpolation
            ...
            for(int i = 1; i <= nA; i++) {
                for(int j = 1; j <= nB; j++) {
                    ...
                    for(int lm=0; lm < num_inter; lm++) {
                        ...
                        int idx = my * NET_RESOLUTION_WIDTH + mx; //point (mx, my)
    ...
    for(int i = 0; i < subset.size(); i++) {
        if (subset[i][SUBSET_CNT]>=global.connect_min_subset_cnt && (subset[i][SUBSET_SCORE]/subset[i][SUBSET_CNT])>global.connect_min_subset_score) {
            for(int j = 0; j < num_parts; j++) {
                int idx = int(subset[i][j]); //index
                if (idx) {
                    joints[cnt*num_parts*3 + j*3 +2] = peaks[idx]; //peaks confidence map
...
int connectAILimbs( //TODO  
    ...
    ModelDescriptor *model_descriptor) {
        ...
        CHECK_EQ(num_parts, 14);
        CHECK_EQ(number_limb_seq, 13);

        int peaks_offset = 3*(max_peaks+1);
        subset.clear();
        connection.clear();
// 26 channels for limb
        for(int k = 0; k < number_limb_seq; k++) { //13
            const float* map_x = heatmap_pointer + mapIdx[2*k] * NET_RESOLUTION_HEIGHT * NET_RESOLUTION_WIDTH;
            const float* map_y = heatmap_pointer + mapIdx[2*k+1] * NET_RESOLUTION_HEIGHT * NET_RESOLUTION_WIDTH;

            const float* candA = peaks + limbSeq[2*k]*peaks_offset; //for candA point
            const float* candB = peaks + limbSeq[2*k+1]*peaks_offset;
//output candA and candB and write out to frame
            std::vector< std::vector<double> > connection_k;
            int nA = candA[0];
            int nB = candB[0];
            ...
            for(int i = 1; i <= nA; i++) {
                for(int j = 1; j <= nB; j++) {
                    ...
                    for(int lm=0; lm < num_inter; lm++) {
                        ...
                        float score = (vec_x*map_x[idx] + vec_y*map_y[idx]);
                        VLOG(5) << "inter:" << lm << "score:" << score << "connect_inter_thre: " << global.connect_inter_threshold;
                        if (score > global.connect_inter_threshold) {
                            sum = sum + score;
                            count ++;
                        }
                    ...
                    //float score = sum / count; // + std::min((130/dist-1),0.f)
                    // add log info
                    VLOG(5) << "limb" << k << "na, nb" << i << " ," << j << " count:" << count;
                    if (count > global.connect_inter_min_above_threshold) {//num_inter*0.8) { //thre/2
...
// output connection_k debug info and 
        VLOG(5) << "connection_"<<k << " size: " << connection_k.size();

// connect all k limbs to different subset of different person
        if (k==0) {
            std::vector<double> row_vec(num_parts+3, 0);
            for(int i = 0; i < connection_k.size(); i++) {
                double indexA = connection_k[i][0];
...
// output different subset on page
    VLOG(5) << "start deleting subset:";
    //** joints by deleting some rows of subset which has few parts occur
    int cnt = 0;
    for(int i = 0; i < subset.size(); i++) {
        VLOG(5) << "subset_cnt, subset_score " << subset[i][SUBSET_CNT] << " " << subset[i][SUBSET_SCORE]/subset[i][SUBSET_CNT] 
                << "threshold: " << global.connect_min_subset_cnt << " " << global.connect_min_subset_score;
        if (subset[i][SUBSET_CNT]>=global.connect_min_subset_cnt && (subset[i][SUBSET_SCORE]/subset[i][SUBSET_CNT])>global.connect_min_subset_score) {
            for(int j = 0; j < num_parts; j++) {
...
void* processFrame(void *i) { //to get joints by using subset and connection
    ...
    std::vector< std::vector<double>> subset; // todo 
    std::vector< std::vector< std::vector<double> > > connection;

    const boost::shared_ptr<caffe::Blob<float>> heatmap_blob = net_copies[tid].person_net->blob_by_name("resized_map"); // resized_map for concat parts and limbs 
    const boost::shared_ptr<caffe::Blob<float>> joints_blob = net_copies[tid].person_net->blob_by_name("joints"); // joints for all joints divided by person

    caffe::NmsLayer<float> *nms_layer = (caffe::NmsLayer<float>*)net_copies[tid].person_net->layer_by_name("nms").get();  //TODO nms
    ...
    while(1) {
        ...
        VLOG(2) << "NMS_LAYER threshold: " << global.nms_threshold;
        net_copies[tid].person_net->ForwardFrom(0); // forward process image 
        VLOG(2) << "CNN time GET heat_map and vec_map" << (get_wall_time()-frame.gpu_fetched_time)*1000.0 << " ms.";
        //cudaDeviceSynchronize();
        float* heatmap_pointer = heatmap_blob->mutable_cpu_data(); // all heatmap;
        const float* peaks = joints_blob->mutable_cpu_data(); //for peaks of each num_parts

        float joints[MAX_NUM_PARTS*3*MAX_PEOPLE]; //10*15*3 //todo 70*3*96

        int cnt = 0;
        //CHECK_EQ(net_copies[tid].nms_num_parts, 14);
        double tic = get_wall_time();
        const int num_parts = net_copies[tid].nms_num_parts;
        if (net_copies[tid].nms_num_parts==14) {
            cnt = connectAILimbs(subset, connection,
                                                 heatmap_pointer, peaks,
                                                 net_copies[tid].nms_max_peaks, joints, net_copies[tid].up_model_descriptor.get()); //todo 
        } else if (net_copies[tid].nms_num_parts==15) {
        ...
        } // TODO using  part candidates to connect
        ...
        if (subset.size() != 0) { 
            ...
            for(int n = 0; n < valid_data; n++) {
                frame_batch[n].numPeople = net_copies[tid].num_people[n]; // n =0
                ...
                for (int ij=0;ij<frame_batch[n].numPeople*num_parts*3;ij++) {
                    frame_batch[n].joints[ij] = joints[ij]; //joints for parts and limb sequence  // joints TODO to the last result
                }


                cudaMemcpy(frame_batch[n].data_for_mat, net_copies[tid].canvas, DISPLAY_RESOLUTION_HEIGHT * DISPLAY_RESOLUTION_WIDTH * 3 * sizeof(float), cudaMemcpyDeviceToHost); //device mean gpu, host means cpu
...
void* postProcessFrame(void *i) { // assign data_for_wrap
    //int tid = *((int *) i);
    Frame frame;
        ...
        for(int c = 0; c < 3; c++) {
            for(int i = 0; i < DISPLAY_RESOLUTION_HEIGHT; i++) {
                for(int j = 0; j < DISPLAY_RESOLUTION_WIDTH; j++) {
                    ...
                    value = value<0 ? 0 : (value > 255 ? 255 : value); //normalize and transform to rgb
...
void* displayFrame(void *i) { //single thread to show frame.data_for_wrap
    Frame frame;
    int counter = 1;
...
// to show some debug text
        ...
        delete [] frame.data_for_mat; // todo 
        delete [] frame.data_for_wrap;
        delete [] frame.data;
...
bool handleKey(int c) {
    ...
    if (c=='-' || c=='=') {
        if (c=='-')
            global.nms_threshold -= 0.005; //0.005
        if (c=='=')
...
// Tee global parameters must be assign after the main has started, not statically before. Otherwise, they will take the default values, not the user-introduced values.
