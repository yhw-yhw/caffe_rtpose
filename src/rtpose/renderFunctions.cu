...
#define LIMB_AI {12,13, 0,1,1,2, 3,4, 4,5, 6,7, 7,8, 9,10, 10,11}
//#define LIMB_AI {12,13, 3,4, 4,5, 0,1, 1,2, 9,10, 10,11, 6,7, 7,8}
...
inline __device__ void getColor(float* c, float v, float vmin, float vmax) //heat map
{
   ...
   if (v < (vmin + 0.125 * dv)) { //1/8
      c[0] = 256 * (0.5 + (v * 4)); //B: 0.5 ~ 1
      c[1] = c[2] = 0;
   } else if (v < (vmin + 0.375 * dv)) { //3/8
      c[0] = 255;
      c[1] = 256 * (v - 0.125) * 4; //G: 0 ~ 1
      c[2] = 0;
   } else if (v < (vmin + 0.625 * dv)) { //5/8
      c[0] = 256 * (-4 * v + 2.5);  //B: 1 ~ 0
      c[1] = 255;
      c[2] = 256 * (4 * (v - 0.375)); //R: 0 ~ 1
   } else if (v < (vmin + 0.875 * dv)) { // 7/8
      c[0] = 0;
...
void render_mpi_parts(float* canvas, int w_canvas, int h_canvas, int w_net, int h_net,
  ...
  //canvas, image    in width * height * 3 * N //N for num frame
...
__global__ void render_pose_27parts(float* dst_pointer, int w_canvas, int h_canvas, float ratio_to_origin,
                             float* poses, int boxsize, int num_people, float threshold){
   const int NUM_PARTS = 14;
...
// for each limb to render
    for(int p = 0; p < num_people; p++){
      for(int l = 0; l < nlimb; l++){
        ...
        float value_b = shared_poses[p*NUM_PARTS*3 + part_b*3 + 2];
        if(value_a > threshold && value_b > threshold){
          float x_p = (x_a + x_b) / 2; //limb midterm
          float y_p = (y_a + y_b) / 2;
          float angle = atan2f(y_b - y_a, x_b - x_a);
          float sine = sinf(angle);
          float cosine = cosf(angle);
      ...
      for(int i = 0; i < NUM_PARTS; i++) { //for every point
        ...
        if(value > threshold) {
          if((x - pose_x_on_image) * (x - pose_x_on_image) + (y - pose_y_on_image) * (y - pose_y_on_image) <= radius * radius){
            ...
            b = 0.4 * b + 0.6 * color[(i%9)*3+2]; //0.6 0.4
            g = 0.4 * g + 0.6 * color[(i%9)*3+1];
            r = 0.4 * r + 0.6 * color[(i%9)*3];
          }
        }
...
//for AIchallenger
__global__ void render_pose_27parts_heatmap(float* dst_pointer, int w_canvas, int h_canvas, int w_net,
                                            int h_net, float* heatmaps, int num_people, int part){
  ...
  if(x < w_canvas && y < h_canvas){
    ...
    float c[3];
    if (part<15){ //16
      getColor(c, value, 0, 1); //value for  part heatmap
    } else {
      getColor(c, value, -1, 1);
    }
    b = 0.5 * b + 0.5 * c[0];
...
// AI parts:
void render_AI_parts(float* canvas, int w_canvas, int h_canvas, int w_net, int h_net,
                    float* heatmaps, int boxsize, float* centers, float* poses, std::vector<int> num_people, int part){
  //canvas, image    in width * height * 3 * N //N for num frame
  //heatmaps         in w_net * h_net * 15 * (P1+P2+...+PN)
  //centers          in 2 * 11 * 1 * N
  ...
  for(int i = 0; i < N; i++){ //N is always 1 for website
    ...
    if(num_people_this_frame != 0){
      if(part == 0){
        // render_pose_website<<<threadsPerBlock, numBlocks>>>
        VLOG(4) << "num_people_this_frame: " << num_people_this_frame << " ratio_to_origin: " << ratio_to_origin;
        render_pose_27parts<<<threadsPerBlock, numBlocks>>>(canvas, w_canvas, h_canvas, ratio_to_origin,
                                                            poses+offset_pose_so_far, boxsize,
                                                            num_people_this_frame, threshold); //render all parts and all limbs
      }
      else if (part > 0) {
        //render_pose_website_heatmap<<<threadsPerBlock, numBlocks>>>
        //LOG(ERROR) << "GPU part num: " << part-1;
        render_pose_27parts_heatmap<<<threadsPerBlock, numBlocks>>>(canvas, w_canvas, h_canvas, w_net, h_net,
                                                          heatmaps+offset_heatmap_so_far, num_people_this_frame,
                                                          part-1); //render only the part 
      }
    } else {
      if (part > 0) {
        render_pose_27parts_heatmap<<<threadsPerBlock, numBlocks>>>(canvas, w_canvas, h_canvas, w_net, h_net,
