...
void NmsLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top){
	...
	threshold_ = nms_param.threshold(); //for regard the point score is lower than threshold
...
void NmsLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top){
	const int num = bottom[0]->shape(0);  // 15+26 = 41
        LOG(INFO) << "num channels for nsm layer :" << num;
	...
	for(int n = 0; n < num; n++){
		...
		dst_pointer[n*offset2_dst] = peakCount; //0, for peakCount, 1 for nothing, than x,y x2,y2 ...
	}
}
